...
import { Avatar, Button, ButtonGroup, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tooltip } from '@mui/material';
                        ...
                        <div className='bg-white custom-box-shadow mt-3'>
                            <TableContainer>
                                <Table aria-label="simple table">
                                    <TableHead>
                                        <TableRow>
                                            <TableCell className='fw-bold bg-secondary-subtle'>#</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="">Product Name</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="right">Price</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="right">Discount</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="right">Qty</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="right">Total</TableCell>
                                            <TableCell className='fw-bold bg-secondary-subtle' align="end">Remove</TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {
                                            Array?.isArray(cartData) && cartData?.length > 0 ?
                                                Array?.isArray(cartData) && cartData?.map((val,key) => (
                                                    <TableRow key={key}>
                                                        <TableCell component="th" scope="row" onClick={()=>navigate(`/product-detail/${val?.productId}`)}>
                                                            <Avatar alt={val?.productImageUrl} src={val?.productImageUrl} />
                                                        </TableCell>
                                                        <TableCell className='fw-bold' onClick={()=>navigate(`/product-detail/${val?.productId}`)}>{val?.productName}</TableCell>
                                                        ...
                                                        <TableCell align="end">
                                                            <Tooltip title="Remove" onClick={()=>removeProductFromCart(val?._id)}>
                                                                <IconButton>
                                                                    <HighlightOffIcon role='button' className='text-primary' />
                                                                </IconButton>
                                                            </Tooltip>
                                                        </TableCell>
                                                    </TableRow>
                                            ...
                                            : 
                                            <TableRow>
                                                <TableCell align="center" colSpan={7}>No Product Added</TableCell>
                                            </TableRow>
                                        }
                                        {
                                            Array?.isArray(cartData) && cartData?.length > 0 ?
                                                <TableRow>
                                                    <TableCell sx={{ textAlign: 'right', fontWeight: 'bold' }} colSpan={6}>Grand Total</TableCell>

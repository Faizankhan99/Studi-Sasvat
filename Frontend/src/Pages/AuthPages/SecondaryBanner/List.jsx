...
import { deleteSecondaryBanner, getSecondaryBanners, resetSecondaryBannerState } from '../../../Features/SecondaryBanner/SecondaryBannerSlice'
...
const SecondaryBannerList = () => {
    ...
    const filteredData = list?.filter(item =>
        item?.secondaryBannerImageUrl?.toLowerCase().includes(searchInput.toLowerCase())
    );
    ...
    const handleDelete = (imageId) => {
        ...
        if (isConfirmed) {
            setTimeout(() => {
                ...
                dispatch(deleteSecondaryBanner(imageId))
            }, 500)
        }
    }
    ...
    useEffect(()=>{
        if(responseStatus == 'success' && responseMessage == 'Get All'){
            setList(secondaryBanners?.data)
            setTimeout(() => {
                setFullPageLoading(false)
            }, 1000);
        }
        if(responseStatus == 'success' && responseMessage == 'Secondary Banner deleted successfully'){
            dispatch(getSecondaryBanners())
        }
                                            ...
                                            Array?.isArray(currentItems) && currentItems?.map((val,key)=>(
                                                <tr key={key}>
                                                    ...
                                                    <td className='p-2 text-nowrap' role='button' onClick={()=>handleDetail(val)}>
                                                        <Avatar
                                                            alt={val?.secondaryBannerImageUrl}
                                                            src={val?.secondaryBannerImageUrl}
            ...
            >
                <Box sx={style}>
                    <Typography id="modal-modal-description">
                        ...
                        <img src={detailedData?.secondaryBannerImageUrl} className='w-100' alt={detailedData?.secondaryBannerImageUrl} />
                    </Typography>
                </Box>
            </Modal>
        </>
